...
const ReminderHistory = () => {
  ...
  return (
    <div className={`space-y-5 ${user_selector?.role === "client" ? "p-4 md:p-6" : ""}`}>
      ...
      <div
        className="flex items-center justify-between sm:flex-col md:flex-row lg:flex-row gap-4 mb-2"
        style={{ marginTop: 0 }}
      >
        <div className="flex items-center gap-2 w-[50%]">
          <Input
            placeholder={
              user_selector.role === "client"
                ? "Search Queries, Subjects, or Details..."
                : "Search Clients, Subjects,ACCT. Manager or Details..."
            }
        ...
        >
          ...
          <Option value="Show only opened">Show only opened</Option>
          <Option value="Show only unopened">Show only unopened</Option>
        </Select> */}
      </div>

      <div style={{ fontSize: "14px", color: "#8c8c8c", margin: "0" }}>
        Showing {filteredReminderHistory.length} of {reminderHistory.length} reminders
      </div>
      ...
      <div className="reminder-history-table" style={{ marginTop: 0 }}>
        ...
        <Table
          columns={columns}
          dataSource={filteredReminderHistory}
          pagination={false}
          loading={loading}
          rowKey="key"
          // bordered
